...
type ConnectionPool = (MongoConnPool, String)
...
-- them yourself.
withConnectionPool :: (MonadControlIO m, NetworkIO m) => ((MongoConnPool, String) -> m a) -> m a
withConnectionPool = withMongoDBConn connectionCount dbName connStr 

runConnectionPool :: (MonadControlIO m, NetworkIO m) => MongoDBReader MongoHost m a -> (MongoConnPool, String) -> m a
runConnectionPool = runMongoDBConn 
